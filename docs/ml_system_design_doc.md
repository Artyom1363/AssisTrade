...
Из основных очевидных проблем - полное отсутсвие данных, т.к. проект новый. Для базовой оценки сформируем корзинки запросов. Пока данных для обучения нет - будем использовать LLM по API. Постепенно заменять синтетику реальными данными. Также копить данные для обучения.
...
# TODO:
- Добавить сслыку на спаршенную документацию метамаска
- Добавить метрики для каждой из компонент
- Больше информации о том, где и что для бейзлайна, а где и что для дальнейшнего развития
- Описание процесса запуска пилота (пользователю надо сказать, что он должен все писать в одно сообщение)
- Добавить картинку с микросервисами