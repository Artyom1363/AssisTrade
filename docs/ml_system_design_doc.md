...
Из основных очевидных проблем - полное отсутсвие данных, т.к. проект новый. Для базовой оценки сформируем корзинки запросов. Пока данных для обучения нет - будем использовать LLM по api.
