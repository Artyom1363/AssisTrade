...
**Появнение задания:** хотя не менее важной задачей является задача положить средства на кошелек, 5$ будут переведены пользователю, для экономии его средств и времени. Задача оставить ровно 70% ставится из соображений, что нужно учитывать цену газа.


**Ограничения на сервис:**
1. Бюджет на api на одного пользователя: 2$
2. На время проведения пилота требования по SLA следующие: 80% ответов < 8 с, 99.9% ответов < 10 секунд.
...
На текущий момент время распределяется следующим образом:
1. Запрос в гигачат
2. Получение эмбеда с помощью bge-m3

Самая проблема масштабируемости на текущий момент - увеличение кол-ва одновременных запросов в gigachat. Эту проблему можно решить перейдя на другой тариф "Для бизнеса". Если все равно будем упираться в ограничения: можно рассмотреть вариант сменить модель (перейти на yandexgpt) или самостоятельно развернуть одну из открытых моделей, например deepseek или qwen (скорее всего качество будет не сильно хуже). Сделать это можно как купив собственную видеокарту, так и взяв в аренду, зависит от перспектив развития сервиса в этот момент.

Вторая проблема - получение эмбедингов с помощью bge-m3 - это проблему можно решить для начала увеличив кол-во cpu. Если этого будет не достаточно, можно арендовать видеокарту или купить собственную.

  
**4.3. Требования к работе системы**
  
В первое время высокой нагрузки не ожидается. Однако стоит предусмотреть обработку запросов как минимум от 3 пользователей параллельно. SLA для production при работе с не более 3 пользователями одновременно: 80% ответов < 3 секунд, 99.9% ответов < 5 секунд (разные интенты могут работать с разной скоростью).
...
На текущий момент из чувствительных данных только адреса кошельков и имена контактов. Приватные ключи не хранятся. Эти данные сейчас не удовлетворяют всем требованиям 152-ФЗ, в дальнейшем это будет исправлено.
...
Сервисы взаимодействуют между собой по API. Ознакомиться в api можно по ссылке http://45.81.35.238:8001/docs
