...
Для начала нужно выбрать группу пользователей и случайно разбить их на 2 подгруппы. 
...
Далее проанализируем трудности с которыми сталкивались пользователи из второй группы. Исправим их и возмем еще одну группу и попросим ее выполнить ту же задачу, но мы только расскажем о наличии нашего сервиса без обязательства пользоваться им. Это нам покажет почему пользователь может бросить на "пол пути" использование нашего сервиса, а доведет свое действие до конца.
